...
class MemoryDILU(MemoryBase):
    ...
    def addMemory(self, current_situation):
        ...
        task_name = re.search(r"Your task is to:\s*(.*?)\s*>", current_situation).group(
            1
        )
...
Score: """
            ...
            response = llm_response(
                prompt=prompt, model=self.llm_type, temperature=0.1, stop_strs=["\n"]
            )
            score = (
...
class MemoryTP(MemoryBase):
    ...
    def retriveMemory(self, query_scenario):
        ...
        experience_plans = []
        task_description = (
            "You are in the" + query_scenario.rsplit("You are in the", 1)[1]
        )
...
"""
            experience_plans.append(
                llm_response(prompt=prompt, model=self.llm_type, temperature=0.1)
            )
...
class MemoryVoyager(MemoryBase):
    ...
    def addMemory(self, current_situation):
        ...
        trajectory_summary = llm_response(
            prompt=prompt, model=self.llm_type, temperature=0.1
        )
...
class MemoryContextual(MemoryBase):
    def __init__(self, llms_type) -> None:
...
Score: """
    ...
    def addMemory(self, current_situation):
        ...
        environment = current_situation.split(
            "You are in the middle of a room. Looking quickly around you, you see"
        )[1].split("Your task is to:")[0]
